...
def _load_toml_config_from_cwd() -> Dict[str, Any]:
    ...
    cwd = Path.cwd()
    candidates = list(cwd.glob(".share-my-repo*.toml")) + list(
        cwd.glob(".*config.toml")
    )
    if not candidates:
        return {}
    ...
    if _toml_lib is None:
        print(
            "TOML parser not available: please install tomli or use Python 3.11+.",
            file=sys.stderr,
        )
        sys.exit(1)
...
def process_repositories(
    ...
    if config:
        ...
        if max_file_size == 16384 and "max_file_size" in config:
            try:
                ...
                print(
                    "Warning: 'max_file_size' in TOML config is not an integer; ignoring.",
                    file=sys.stderr,
                )
        ...
        if output_format == "markdown":
            ...
            if fmt_val:
                ...
                if fmt_val not in VALID_FORMATS:
                    print(
                        f"Warning: unknown format '{fmt_val}' in TOML config; using default.",
                        file=sys.stderr,
                    )
                else:
        ...
        if preview is None and "preview" in config:
            try:
                ...
                print(
                    "Warning: 'preview' in TOML config is not an integer; ignoring.",
                    file=sys.stderr,
                )
    ...
    for path_str in paths:
        ...
        try:
            if path.is_file():
                repo_info = process_single_file(
                    path, file_processor, line_numbers, preview
                )
            elif path.is_dir():
                ...
                print(
                    f"Error: {path} is neither a file nor a directory", file=sys.stderr
                )
                continue
    ...
    try:
        if len(all_results) == 1:
            final_output = formatter.format_output(
                all_results[0], output_format, show_tokens, recent
            )
        else:
            combined_info = combine_repositories(all_results)
            final_output = formatter.format_output(
                combined_info, output_format, show_tokens, recent
            )
...
def process_directory(
    ...
    try:
        files = file_processor.discover_files(
            path, include_patterns, exclude_patterns, use_gitignore=use_gitignore
        )
        if recent:
    ...
    for file_path in files:
        try:
            ...
            if line_numbers and content:
                content = "\n".join(
                    f"{idx + 1}: {line}" for idx, line in enumerate(content.splitlines())
                )
